...
- **task**: タスク管理（CRUD、ステータス遷移）
- **habit**: 習慣エンジン（デイリーレビュー、ストリーク追跡）
  - DailyReview: 日次振り返り（Pending → Completed/Skipped）
  - Streak: 連続達成日数の計算
...
- フロントエンドバインディング: `frontend/wailsjs/go/presenter/{TaskHandler,HabitHandler}.js`
